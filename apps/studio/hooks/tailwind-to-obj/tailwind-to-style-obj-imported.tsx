/* All credit goes to https://github.com/finom for this code.
 * This was pulled from the following repository file:
 * https://github.com/finom/tailwind-to-object/blob/main/index.ts
 * Some modifications have been made to the original.
 * Many more are needed for this to be a drop-in replacement.
 */
...
const getStyles = (): typeof styles => {
  // eslint-disable-next-line @typescript-eslint/no-use-before-define
  const entries = CheatSheet.map(({ content }) =>
    content.map(({ table }) =>
      table.map((list) => {
        const twStyles: Record<string, string> = {};
        const selector = list.length === 3 ? list[0] : list[1];
        const styleStr = list.length === 3 ? list[1] : list[2];

        styleStr.split(';').forEach((style) => {
          const [key, value] = style.split(':');

          if (value) {
            twStyles[camelCase(key.trim())] = value.trim();
          }
        });

        return [selector.slice(1), twStyles] as const;
      })
    )
  ).flat(2);
...
export default function tailwindToObject(tailwind_strings: ClassValue[], breakpoint: breakpointType | undefined) {
  const selector = cn(...tailwind_strings);
  if (!selector) return {};
  if (!styles) {
    styles = getStyles();
  }
  ...
  const styleAttr = {} as React.CSSProperties;
  ...
  selector
    ...
    .forEach((term) => {
      try {
        ...
        const termPartitions = term.split(':');
        const breakpointPartition = (termPartitions.length === 2 && 
                                     BREAKPOINTS_ORDERED.includes(termPartitions[0] as breakpointType)) ? 
                                     termPartitions[0] : 
                                     "xs";
        ...
        if (segments[0] === "space" && segments.length === 3) {
          ...
          //   (segments[1] === "x") ?
          //   {
          //     marginLeft: `${parseFloat(segments[2])/2}rem`
          //   } : {
          //     marginTop: `${parseFloat(segments[2])/2}rem`
          //   }
          // );
          ...
          // Breakpoint-Aware
          Object.assign(
            stylesByBreakpoint[breakpointPartition],
            (segments[1] === "x") ?
            {
              marginLeft: `${parseFloat(segments[2])/2}rem`
            } : {
          ...
          if (s.includes('[')) {
            const property = s.split('-[')[0].replace('.', '');
            const properyValue = s.match(/(?<=\[)[^\][]*(?=])/g)?.[0];
            if (arbitrarySupportedClasses[property as keyof typeof arbitrarySupportedClasses]) {
              // Object.assign(styleAttr, {
              //   [camelCase(arbitrarySupportedClasses[property as keyof typeof arbitrarySupportedClasses])]: properyValue + (important ? ' !important' : ''),
              // });

              // Breakpoint-Aware
              Object.assign(stylesByBreakpoint[breakpointPartition], {
                [camelCase(arbitrarySupportedClasses[property as keyof typeof arbitrarySupportedClasses])]: properyValue + (important ? ' !important' : ''),
              });
            } else {
  ...
  // console.log("Breakpoint Styles:", stylesByBreakpoint);
	...
	for (let i = 0; i <= BREAKPOINTS_ORDERED.indexOf(breakpoint); i++) {
		// console.log(`Assigning breakpoint style ${BREAKPOINTS_ORDERED[i]} to styleAttrBreakpointAware:`, JSON.parse(JSON.stringify(stylesByBreakpoint[BREAKPOINTS_ORDERED[i]])));
		Object.assign(styleAttr, stylesByBreakpoint[BREAKPOINTS_ORDERED[i]]);
...
// I'm sorry, I don't remember where did I get that from. That's old code from forgotten project.
// Please, if you know the source, let me know.
const CheatSheet = [
  {
    title: 'Layout',
    content: [
