# This is a rewrite of exllama's model.py module.
# The sole change is the last third or so of the ExLlama class'
# __init__ function has been moved to another function called
# reload_params. This was necassary for efficiently changing
# context size after initializing a model without rereading
# weights from the disk. As of now, it takes about 120 ms to
# change context size in the model on llama2_chat_7b.
...
class ExLlama:
    ...
    def __init__(self, config):
        ...
        f = None
        prev_path = ""
        for key, path in self.load_keys.items():
            ...
            if key.endswith(".qweight"): self.max_dq_buffer_size = max(self.max_dq_buffer_size, keep_tensor.numel() * 8)

            self.tensors[key] = keep_tensor
        ...
        self.lm_head.weight = nn.Parameter(self.tensors["lm_head.weight"])
        ...
        self.embed_tokens.weight = nn.Parameter(self.tensors["model.embed_tokens.weight"])
        with torch.no_grad():
            self.embed_tokens.weight[self.config.pad_token_id] = 0

        # Norm

        self.norm = ExLlamaRMSNorm(self.config, self.tensors, "model.norm.weight")
    ...
    def reload_params(self) -> None:
        ...
        for i in range(self.config.num_hidden_layers):
            ...
            layer = ExLlamaDecoderLayer(self.config, self.tensors, f"model.layers.{i}", i, sin, cos)
        ...
        for dev in self.config.device_map.get_layers_devs():
            ...
            temp_state = torch.zeros((self.config.max_input_len, self.config.intermediate_size), dtype = torch.float16, device = dev)
            temp_mlp = torch.zeros((self.config.fused_mlp_thd * 2, self.config.intermediate_size), dtype = torch.float16, device = dev)
            temp_zeros_float = torch.zeros((1, 65536), dtype = torch.float32, device = dev)
            temp_dq = torch.zeros((1, self.max_dq_buffer_size), dtype = torch.float16, device = dev)
