...
async def find_equation_blocks(page, processor, get_total_texify_tokens_override=None):
    equation_blocks = []
    ...
    # TODO: Parallelize the dependency async case.
    ...
    for region_idx, equation_region in enumerate(equation_regions):
        if region_idx not in equation_lines or len(equation_lines[region_idx]) == 0:
            ...
            if not get_total_texify_tokens_override is None:
                if inspect.iscoroutinefunction(get_total_texify_tokens_override):
                    total_tokens = await get_total_texify_tokens_override(
                        block_text, processor
                    )
                else:
                    total_tokens = get_total_texify_tokens_override(
                        block_text, processor
                    )
            else:
...
async def insert_latex_block(
    page_blocks: Page, page_equation_blocks, predictions, pnum, processor, get_total_texify_tokens_override=None
):
    converted_spans = []
    idx = 0
    success_count = 0
    ...
    if not get_total_texify_tokens_override is None:
        block_texify_conditions = await gather(*[get_total_texify_tokens_override(
            predictions[block_number], processor
        ) for block_number in range(len(page_equation_blocks))])
    else:
        block_texify_conditions = []
    ...
    for block_number, (insert_block_idx, insert_line_idx, token_count, block_text, equation_bbox) in enumerate(page_equation_blocks):
        ...
        if not get_total_texify_tokens_override is None:
            sub_condition = block_texify_conditions[block_number] < settings.TEXIFY_MODEL_MAX
        else:
            sub_condition = get_total_texify_tokens(latex_text, processor) < settings.TEXIFY_MODEL_MAX
        
        conditions = [
            sub_condition,  # Make sure we didn't get to the overall token max, indicates run-on
...
async def replace_equations(
    doc, pages: List[Page], texify_model, batch_multiplier=1,
    run_texify_batch_recognition=None,
    get_total_texify_tokens=None
):
    unsuccessful_ocr = 0
    successful_ocr = 0

    try:
        tex_processor = texify_model.processor
    except:
        tex_processor = None
    
    
    # Find potential equation regions, and length of text in each region
    equation_block_results = await gather(*[find_equation_blocks(
        page, tex_processor, get_total_texify_tokens_override=get_total_texify_tokens
    ) for page in pages])
    
    equation_blocks = []
    for pnum, page in enumerate(pages):
        equation_blocks.append(equation_block_results[pnum])
    ...
    if not run_texify_batch_recognition is None:
        if inspect.iscoroutinefunction(run_texify_batch_recognition):
            predictions = await run_texify_batch_recognition(
                images, token_counts, texify_model, batch_multiplier=batch_multiplier
            )
        else:
            predictions = run_texify_batch_recognition(
                images, token_counts, texify_model, batch_multiplier=batch_multiplier
            )
    else:
    ...
    for page_idx, page_equation_blocks in enumerate(equation_blocks):
        ...
        success_count, fail_count, converted_span = await insert_latex_block(
            pages[page_idx],
            page_equation_blocks,
            page_predictions,
            page_idx,
            tex_processor,
            get_total_texify_tokens_override=get_total_texify_tokens
        )
